...
impl LocalPoolHandle {
    ...
    pub fn new(pool_size: usize) -> LocalPoolHandle {
        Self::new_with_runtime_creator(pool_size, || {
    ...
    #[track_caller]
    pub fn new_with_runtime_creator<F>(pool_size: usize, mut creator: F) -> LocalPoolHandle
    where
        F: FnMut() -> tokio::runtime::Runtime,
    {
        assert!(pool_size > 0);

        let workers = (0..pool_size)
            .map(|_| LocalWorkerHandle::new_worker(creator()))
            .collect();
...
impl LocalWorkerHandle {
    ...
    fn new_worker(runtime: tokio::runtime::Runtime) -> LocalWorkerHandle {
        ...
        std::thread::spawn(move || Self::run(runtime, receiver, task_count_clone));
