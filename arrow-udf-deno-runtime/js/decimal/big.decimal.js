// Taken fom https://github.com/royNiladri/js-big-decimal
//function add {
function add(number1, number2) {
  ...
  (_a = pad(trim(number1), trim(number2))),
    (number1 = _a[0]),
    (number2 = _a[1]);
  if (neg == 1) {
  ...
  if (pt_dvsr >= 0) {
    ...
    if (pt_dvnd == -1) {
      ...
      if (pt_dvsr > pt_dvnd) {
        ...
        dividend = trim(
          dividend.substring(0, loc) + "." + dividend.substring(loc)
        );
      } else if (pt_dvsr == pt_dvnd) {
...
function modulus(dividend, divisor) {
  ...
  var result = subtract(
    ...
    multiply(
      divisor,
      roundOff(divide(dividend, divisor), 0, RoundingModes.FLOOR)
    )
  );
  return sign + result;
}
...
var bigDecimal = /** @class */ (function () {
  ...
  bigDecimal.prototype.sub = bigDecimal.prototype.subtract;
  ...
  bigDecimal.prototype.mul = bigDecimal.prototype.multiply; 
  ...
  bigDecimal.prototype.div = bigDecimal.prototype.divide;
